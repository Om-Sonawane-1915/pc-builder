...
function App() {
  ...
  function handleCPUCompare() {
    ...
    compareCPUs(cpuCompare1, cpuCompare2).then(setComparison);
  }

  function handleGPUCompare() {
    if (!gpuCompare1 || !gpuCompare2) return;
    compareGPUs(gpuCompare1, gpuCompare2).then(setComparison);
  }

  async function handleAutoBuild() {
    const data = await generateBuild(budget, purpose);
  ...
  function exportPDF() {
    ...
    doc.setFontSize(12);
    doc.text(
      `Compatibility: ${result.compatible ? "Compatible" : "Issues Found"}`,
      20,
      finalY + 12
    );
    doc.text(`Power Required: ${result.required_power}W`, 20, finalY + 20);

    const cleanTier = result.overall_score.tier.replace(/[^\x00-\x7F]/g, "");
    doc.text(`Build Tier: ${cleanTier}`, 20, finalY + 28);
    doc.text(`Overall Score: ${result.overall_score.score}/100`, 20, finalY + 36);

    const cleanRating = result.overall_score.rating.replace(/[^\x00-\x7F]/g, "");
    doc.text(`Rating: ${cleanRating}`, 20, finalY + 44);
  ...
  return (
    <div className="container">
      ...
      <p className="subtitle">Build your perfect PC with compatible components.</p>
...
/>
      ...
      <div className="selection-overview">
        ...
        <p><strong>CPU ID:</strong> {selectedCPU || "None"}</p>
        <p><strong>GPU ID:</strong> {selectedGPU || "None"}</p>
        <p><strong>Motherboard ID:</strong> {selectedMotherboard || "None"}</p>
        <p><strong>RAM ID:</strong> {selectedRAM || "None"}</p>
        <p><strong>Storage ID:</strong> {selectedStorage || "None"}</p>
        <p><strong>PSU ID:</strong> {selectedPSU || "None"}</p>
      </div>

      <div className="action-bar">
        <button onClick={handleBuild} className="btn-primary">🔨 Build PC</button>
        <button onClick={handleSaveBuild} className="btn-success">💾 Save Build</button>
        <button onClick={exportPDF} className="btn-secondary">📄 Export Build Report</button>
        <button onClick={handleAutoBuild} className="btn-accent">✨ Auto Generate Build</button>
      </div>

      <h2 style={{ marginTop: "40px" }}>⚔️ Component Comparison</h2>
      ...
      <div className="budget-card">
        ...
        <button style={{ marginTop: "14px" }} onClick={handleCPUCompare}>
          Compare CPUs
        </button>
      </div>
      ...
      <div className="budget-card" style={{ marginTop: "25px" }}>
        ...
        <button style={{ marginTop: "14px" }} onClick={handleGPUCompare}>
          Compare GPUs
        </button>
      </div>
      ...
      {comparison && (
        <div className="summary-card" style={{ marginTop: "30px" }}>
          ...
          <div className="summary-table">
            <div className="summary-item summary-item-header">
              <span>Metric</span>
            ...
            <div className="summary-item">
              <span>Price</span>
              <span>₹{Number(comparison.component1.price).toLocaleString("en-IN")}</span>
              <span>₹{Number(comparison.component2.price).toLocaleString("en-IN")}</span>
            </div>
          </div>

          <h2 className="winner-tag">
        ...
        <div className="analysis-section">
          ...
          <div className="analysis-grid">
            <div className="analysis-card compatibility-card">
              ...
              <div>
                <h3>Compatibility</h3>
                <strong className={result.compatible ? "status-good" : "status-bad"}>
            ...
            <div className="analysis-card">
              ...
              <div>
                ...
                <strong>{result.required_power}W</strong>
              </div>
            </div>
            ...
            <div className="analysis-card">
              ...
              <div>
                <h3>Total Price</h3>
                <strong>₹{Number(result.total_price).toLocaleString("en-IN")}</strong>
              </div>
            </div>
            ...
            <div className="analysis-card">
              ...
              <div>
                <h3>Build Tier</h3>
                <strong>{result.overall_score.tier}</strong>
              </div>
            </div>
          </div>
          ...
          <div className="analysis-panel">
            ...
            <div className="component-summary-grid">
              <div className="component-summary">
                ...
                <small>{result.build.cpu.cores} Cores • {result.build.cpu.threads} Threads</small>
                <small>Gaming Score: {result.build.cpu.gaming_score}/100</small>
              </div>

              <div className="component-summary">
                <span>🎮 GPU</span>
                <strong>{result.build.gpu.name}</strong>
                <small>VRAM: {result.build.gpu.memory} GB</small>
                <small>Performance: {result.build.gpu.performance_score}/100</small>
              </div>

              <div className="component-summary">
                <span>🟩 Motherboard</span>
                <strong>{result.build.motherboard.name}</strong>
                <small>Socket: {result.build.motherboard.socket}</small>
                <small>RAM: {result.build.motherboard.ram_type}</small>
              </div>

              <div className="component-summary">
                <span>⚡ RAM</span>
                <strong>{result.build.ram.name}</strong>
                <small>{result.build.ram.capacity} GB</small>
                <small>{result.build.ram.speed} MHz</small>
              </div>

              <div className="component-summary">
                <span>💾 Storage</span>
                <strong>{result.build.storage.name}</strong>
                <small>{result.build.storage.capacity} GB</small>
                <small>{result.build.storage.type}</small>
              </div>

              <div className="component-summary">
                <span>🔌 PSU</span>
                <strong>{result.build.psu.name}</strong>
                <small>{result.build.psu.wattage}W</small>
              </div>
            </div>
          </div>
          ...
          <div className="analysis-panel">
            ...
            {result.total_price <= budget ? (
              <div className="budget-status success">
                ...
                <span>₹{(budget - result.total_price).toLocaleString("en-IN")} remaining</span>
              </div>
            ) : (
              <div className="budget-status danger">
                <strong>🔴 Over Budget</strong>
                <span>₹{(result.total_price - budget).toLocaleString("en-IN")} over budget</span>
              </div>
            )}
          </div>
          ...
          <div className="analysis-panel">
            ...
            <div className="game-grid">
              {Object.entries(result.game_fps).map(([game, fps]) => {
                ...
                return (
                  <div className="game-card" key={game}>
                    <strong>{game}</strong>
                    <span className="fps-number">{fps} FPS</span>
          ...
          <div className="analysis-panel">
            ...
            <div className="resolution-grid">
              <div className="resolution-card">
                <span>1080p</span>
                <strong>{result.estimated_fps["1080p"]} FPS</strong>
              </div>
              <div className="resolution-card">
                <span>1440p</span>
                <strong>{result.estimated_fps["1440p"]} FPS</strong>
              </div>
              <div className="resolution-card">
                <span>4K</span>
                <strong>{result.estimated_fps["4k"]} FPS</strong>
              </div>
            </div>
          </div>
          ...
          <div className="analysis-panel score-panel">
            ...
            <div className="tier-text">{result.overall_score.tier}</div>
            <div className="rating-text">{result.overall_score.rating}</div>
          </div>
          ...
          <div className="analysis-panel">
            ...
            <div className="bottleneck-box">
              <div>
                ...
                <strong>{result.bottleneck.percentage}%</strong>
              </div>
              <div>
                <span>Status</span>
                <strong>{result.bottleneck.status}</strong>
              </div>
            </div>
          </div>
      ...
      <h2 style={{ marginTop: "40px" }}>💾 Saved Builds</h2>

      {savedBuilds.length === 0 ? (
        <p className="no-builds-text">No saved builds found.</p>
      ) : (
        <div className="saved-builds-grid">
          {savedBuilds.map((build) => (
            <div key={build.id} className="saved-build-card">
              ...
              <p><strong>Price:</strong> ₹{Number(build.total_price).toLocaleString("en-IN")}</p>
              <p><strong>Purpose:</strong> {build.purpose}</p>

              <div className="saved-build-actions">
                <button onClick={() => handleLoadBuild(build)} className="btn-success">
                  📂 Load
                </button>
                <button onClick={() => handleDeleteBuild(build.id)} className="btn-danger">
                  🗑 Delete
                </button>
              </div>
            </div>
